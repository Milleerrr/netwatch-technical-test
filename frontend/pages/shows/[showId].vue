    ...
    <div class="mx-auto py-10">
        ...
        <div v-else class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="w-full lg:w-2/3">
                <NuxtImg v-if="show?.image" :src="show.image" :alt="show.title"
            ...
            <div class="flex-grow">
                <h1 class="text-4xl font-bold mb-4 text-gray-800">{{ show?.title }}</h1>
                <p class="text-lg mb-6 text-gray-700">{{ show?.overview }}</p>
                <!-- Genres Section -->
                <div class="mb-6">
                    ...
                    <Collapsible v-model:open="isCastOpen" class="space-y-2">
                        ...
                        <CollapsibleContent class="space-y-2">
                            <ScrollArea class="h-[400px] w-full rounded-md border p-4">
                                <ul class="space-y-4">
                                    <li v-for="castMember in show?.cast" :key="castMember.id"
        ...
        <!-- Comments Section -->
        <div v-if="show?.latest_comments && show?.latest_comments.length"
            class="mt-20 bg-white p-6 rounded-lg shadow-md">
            <h2 class="text-2xl font-semibold mb-4 text-gray-800">Comments</h2>
            <ul class="space-y-6">
                <li v-for="comment in show?.latest_comments" :key="comment.id" class="border-b border-gray-200 pb-4">
...
const show = ref<MediaProps>();
const isLoading = ref(true);

const fetchShow = async () => {
    try {
        const response = await axios.get<MediaProps>(`/api/media/${showId}`);
...
const genreList = computed(() => {
    return show.value?.genres?.map((genre) => genre.name).join(', ');
});
...
const tooltipItems = computed(() => [
    {
        ...
        value: show.value ? `${show.value.vote_average}/10` : '',
    },
    {
        name: 'Popularity',
        icon: 'i-mdi-chart-line',
        colorClass: 'text-green-500',
        value: show.value?.popularity ?? '',
    },
    {
        name: 'Like Count',
        icon: 'i-mdi-cards-heart',
        colorClass: 'text-red-500',
        value: show.value?.vote_count ?? '',
    },
]);
</script>